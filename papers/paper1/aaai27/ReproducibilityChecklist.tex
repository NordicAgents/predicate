...
yes
	...
	\ifyespoints{\vspace{1.2em}If yes, please address the following points:}
		...
		relevance, bounded-context, and rule-fragment assumptions are stated in
		the model section and collected again in the supplement.
		...
		correctness, fixed-hop incompleteness, and the maintained-index contract
		are proposition statements.
...
\begin{itemize}
		...
		confidence semantics, fixture families, scale sizes, and repetition
		counts are reported. The manuscript does not reconstruct every
		...
		on-demand, sparse-retrieval, scoring, and scaling paths are included.
		...
		partial. The paper reports Node.js 22.17.1, Sentence Transformers 5.6.1,
		the pinned encoder revision, dependency locks, and the AMD Ryzen
		Threadripper PRO 5955WX timing host. It does not report the host memory
		size and operating-system version.
		...
		triple count, update amplification, and repeated timing are reported.

		\question{This paper states the number of algorithm runs used to compute each reported result}{(yes/no)}
		yes. Deterministic fixture results are exact single executions; scale
		timings use one unrecorded warm-up and 11 measured repetitions.
		...
		encoder revision, rule settings, scale sizes, query counts, and
