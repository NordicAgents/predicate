...
yes. Core synthetic results, controlled semi-natural results, formal
guarantees, limitations, and natural-prevalence non-claims are labeled
separately.
...
yes
	...
	\ifyespoints{\vspace{1.2em}If yes, please address the following points:}
		...
		non-type IRI incompleteness, the maintained-index contract, and bounded exact joint
		selection are proposition statements.
		...
		minimality, exact joint selection, and update-order details, are in the
		...
		exact comparators, joint-selector boundary tests, workload comparison,
		and external runners with frozen reports.
...
\begin{itemize}
		...
		oracle schema, while the DBLP--Google Scholar leg tests controlled
		positive/negative conflicts over independently supplied gold linkage
		topology and natural attributes.
		...
		frozen synthetic and semi-natural paper-facing results are included in
		the code/data archive.
		...
		yes. The external topology and semi-natural studies use the public CC-BY
		release identified by its citation and record input hashes in the
		shipped reports.
...
\begin{itemize}
		...
		class-stress sizes, joint search caps, workload query counts, and
		repetition counts are reported. The manuscript
		...
		yes. Synthetic data generation, external topology processing, and
		semi-natural controlled-label construction are included in the
		code/data archive.
		...
		joint-budget, semi-natural, workload, class-stress, on-demand,
		sparse-retrieval, scoring, and scaling paths are included.
		...
		evaluation explains why complete minimal-witness inclusion, logical
		triple count, UTF-8 bytes, joint witness-union cost, update amplification,
		retained derived entries, materialized conflict pairs, and repeated
		workload timing are reported.
		...
		executions; workload, scale, and class-stress timings use one unrecorded
		warm-up and 11 measured repetitions.
		...
		encoder revision, rule settings, joint-conflict counts and search caps,
		external selection rule, scale and stress sizes, query counts, and
		repetition protocol are in the paper and artifact.
