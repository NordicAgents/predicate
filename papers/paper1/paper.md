...
retrieval: any policy whose neighborhood is generated by something other than IRI adjacency falls
outside its hypothesis. Of those, we evaluate **key-aware** and **literal-aware** traversal empirically
(§5.2) — and key-aware in particular is witness-complete on five of our seven families at a context
cost within $\approx 1.3\times$ of the witness itself, which we report as a tie rather than a win.
**Lexical (BM25), dense, and hybrid retrieval are not evaluated here**; they are outside Prop. 3's
hypothesis and remain an open empirical question, deferred to the baseline study of §8. We flag this
explicitly because it is the sharpest form of our own falsification condition: if a lexical or dense
policy were witness-complete at comparable cost, then "index the relation that connects witnesses"
would degenerate into "index everything shared," and the completeness/budget frontier would have no
interesting structure. What Prop. 3 pins down is narrower and survives that outcome: IRI adjacency,
the default structural signal, is the wrong index for this class of witness.
...
semantic adjudication is future annotation work).

**Chain depth in the wild (and why it decides §6's argument).** Grouping by a single identifier makes
every pair chain-depth $m{=}1$ *by construction*, so the probe above cannot see the regime where witness
indexing beats key-aware retrieval. Two follow-up probes run a union–find over *all* identifiers a record
carries and report the depth distribution. They disagree, and the disagreement is the finding. In the
OSV slice (24,331 records; CVE/GHSA/PYSEC/MAL aliases), **99.5%** of co-referent pairs share a key
directly and **no pair at $m\ge3$ exists at all** — the cross-record mechanism is ubiquitous (29.3% of
components are multi-record) but essentially flat. In a 40,245-record sample of ClinicalTrials.gov
(NCT, sponsor protocol, NIH grant, EudraCT, CTIS ids), co-reference is far rarer (0.5% of components)
but **14.0%** of co-referent pairs require $m\ge2$, and $m{=}3$ occurs. Neither domain exhibits both a
common mechanism and deep chains. The honest reading is that the chain regime is **domain-dependent and
now measured**, not assumed: where it is flat, key-aware retrieval suffices and the index earns nothing
(§6); where it is not, a fixed hop bound cannot be chosen without knowing a depth the query does not
have. The trial probe measures *structure only* — per-version record history has no API v2 endpoint, so
whether those chained records actually disagree is unmeasured and is **not** reported as a conflict rate.

This motivates a released, human-annotated benchmark
...
**When witness indexing earns its cost — measured against the *strongest* baseline, not the weakest.**
A phase diagram is only honest if each axis is scored against the best policy available on that axis,
rather than against whichever policy happens to fail there. Read that way, Table 2 says something
narrower than a per-axis reading suggests. Two of the three axes separate CWI from a *weak* baseline
only: the *graph distance* axis (infinite through a key literal) defeats IRI-BFS, but key-aware
traversal crosses that literal at $k{=}1$ in 8 triples; and the *non-key shared literal* axis inflates
literal-aware retrieval by 7×→20.6× as nk goes 1→3, but leaves key-aware at 11 triples. Against the
strongest neighborhood policy, exactly one axis survives: the **$\sim_K$-chain depth $m$**. There
key-aware is not merely more expensive but *incomplete* below $k=m$ (0/10 at $m{=}2$, 0/6 at $m{=}3$),
and it becomes complete only by buying the whole radius-$m$ ball (95 and 91 triples) where CWI pays
$3m{+}3$ (9 and 12).

On the five non-chain families the honest verdict is a **tie**: key-aware@1 is witness-complete at
4–12 triples against CWI's 2–10, a ratio of roughly $1.3\times$, and CWI additionally carries ~1.89
index writes per assertion that key-aware does not. A deployment that knows its conflicts are
direct-co-key should use key-aware retrieval and no index at all; we say so plainly because the
alternative — quoting the 8-triple tie inside a "8–95 triples" range — would present a tie as a defeat.

**The residual argument is the unsettable $k$.** Key-aware's completeness depends on choosing $k \ge m$
*before* the query, while $m$ is a property of the stored data that the query does not know. Both
settings lose: $k$ too small misses silently (0 recall, no signal), and $k$ large enough to be safe
buys the ball on every query. Over our seven families, the smallest single $k$ that is complete
everywhere is $k=3$, costing **15,245 context triples in total against CWI's 514 — a 29.7× premium at
equal completeness**. That premium, not the per-case 1.3×, is the retrieval-side case for indexing the
witness relation. Its force is proportional to how often $m \ge 2$ actually occurs, and §4.4 now bounds
that empirically in two domains with opposite answers: 0.5% of co-referent pairs in vulnerability
records, 14.0% in clinical-trial registrations. Where the first figure holds, $k{=}1$ is both safe and
cheap and the index is unnecessary; where the second holds, $k{=}1$ silently misses one co-referent pair
in seven. We report both rather than generalizing from either.
